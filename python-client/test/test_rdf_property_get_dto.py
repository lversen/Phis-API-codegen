# coding: utf-8

"""
    OpenSilex API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.4.7-rdg
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.rdf_property_get_dto import RDFPropertyGetDTO

class TestRDFPropertyGetDTO(unittest.TestCase):
    """RDFPropertyGetDTO unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> RDFPropertyGetDTO:
        """Test RDFPropertyGetDTO
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `RDFPropertyGetDTO`
        """
        model = RDFPropertyGetDTO()
        if include_optional:
            return RDFPropertyGetDTO(
                uri = 'http://opensilex.org/custom_object_property',
                domain = 'vocabulary:SensingDevice',
                range = 'vocabulary:ScientificObject',
                parent = 'http://opensilex.org/parent_custom_object_property',
                name = 'custom_object_property',
                comment = 'Description of the property',
                rdf_type = 'owl:ObjectProperty',
                name_translations = {
                    'key' : ''
                    },
                comment_translations = {
                    'key' : ''
                    },
                publisher = openapi_client.models.user_get_dto.UserGetDTO(
                    uri = 'http://opensilex.dev/users#jean.michel.inrae', 
                    email = 'jean.michel@example.com', 
                    language = 'en', 
                    admin = False, 
                    first_name = 'Jean', 
                    last_name = 'Michel', 
                    linked_person = 'http://opensilex.dev/person#Jean.Michel.mistea', 
                    enable = True, 
                    favorites = test, ),
                publication_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                last_updated_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                domain_label = '',
                range_label = ''
            )
        else:
            return RDFPropertyGetDTO(
                uri = 'http://opensilex.org/custom_object_property',
                domain = 'vocabulary:SensingDevice',
                range = 'vocabulary:ScientificObject',
                rdf_type = 'owl:ObjectProperty',
                name_translations = {
                    'key' : ''
                    },
                comment_translations = {
                    'key' : ''
                    },
        )
        """

    def testRDFPropertyGetDTO(self):
        """Test RDFPropertyGetDTO"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
