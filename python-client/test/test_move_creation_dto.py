# coding: utf-8

"""
    OpenSilex API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.4.7-rdg
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.move_creation_dto import MoveCreationDTO

class TestMoveCreationDTO(unittest.TestCase):
    """MoveCreationDTO unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> MoveCreationDTO:
        """Test MoveCreationDTO
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `MoveCreationDTO`
        """
        model = MoveCreationDTO()
        if include_optional:
            return MoveCreationDTO(
                uri = '',
                rdf_type = 'http://www.opensilex.org/vocabulary/oeev#Irrigation',
                start = '2019-09-08T12:00:00+01:00',
                end = '2019-09-08T13:00:00+01:00',
                is_instant = True,
                description = '2019-09-08T13:00:00+01:00',
                targets = [
                    ''
                    ],
                relations = [
                    openapi_client.models.rdf_object_relation_dto.RDFObjectRelationDTO(
                        property = '', 
                        value = '', 
                        inverse = True, )
                    ],
                var_from = 'test:greenHouseA',
                to = 'test:greenHouseB',
                targets_positions = [
                    openapi_client.models.target_position_creation_dto.TargetPositionCreationDTO(
                        target = 'test:plantA', 
                        position = openapi_client.models.position_creation_dto.PositionCreationDTO(
                            point = null, 
                            x = '35', 
                            y = '76', 
                            z = '8611', 
                            text = 'Near the greenhouse door', ), )
                    ],
                publisher = openapi_client.models.user_get_dto.UserGetDTO(
                    uri = 'http://opensilex.dev/users#jean.michel.inrae', 
                    email = 'jean.michel@example.com', 
                    language = 'en', 
                    admin = False, 
                    first_name = 'Jean', 
                    last_name = 'Michel', 
                    linked_person = 'http://opensilex.dev/person#Jean.Michel.mistea', 
                    enable = True, 
                    favorites = test, ),
                publication_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                last_updated_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f')
            )
        else:
            return MoveCreationDTO(
                is_instant = True,
                targets = [
                    ''
                    ],
        )
        """

    def testMoveCreationDTO(self):
        """Test MoveCreationDTO"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
