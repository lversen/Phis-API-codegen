# coding: utf-8

"""
    OpenSilex API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.4.7-rdg
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.facility_update_dto import FacilityUpdateDTO

class TestFacilityUpdateDTO(unittest.TestCase):
    """FacilityUpdateDTO unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> FacilityUpdateDTO:
        """Test FacilityUpdateDTO
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `FacilityUpdateDTO`
        """
        model = FacilityUpdateDTO()
        if include_optional:
            return FacilityUpdateDTO(
                uri = '',
                rdf_type = '',
                name = '',
                organizations = [
                    ''
                    ],
                address = openapi_client.models.facility_address_dto.FacilityAddressDTO(
                    country_name = '', 
                    locality = '', 
                    postal_code = '', 
                    region = '', 
                    street_address = '', 
                    readable_address = '', ),
                geometry = {"coordinates":[43.618316,3.856912],"type":"Point"},
                publisher = openapi_client.models.user_get_dto.UserGetDTO(
                    uri = 'http://opensilex.dev/users#jean.michel.inrae', 
                    email = 'jean.michel@example.com', 
                    language = 'en', 
                    admin = False, 
                    first_name = 'Jean', 
                    last_name = 'Michel', 
                    linked_person = 'http://opensilex.dev/person#Jean.Michel.mistea', 
                    enable = True, 
                    favorites = test, ),
                publication_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                last_updated_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                relations = [
                    openapi_client.models.rdf_object_relation_dto.RDFObjectRelationDTO(
                        property = '', 
                        value = '', 
                        inverse = True, )
                    ],
                rdf_type_name = '',
                sites = [
                    ''
                    ],
                variable_groups = [
                    ''
                    ]
            )
        else:
            return FacilityUpdateDTO(
                uri = '',
        )
        """

    def testFacilityUpdateDTO(self):
        """Test FacilityUpdateDTO"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
