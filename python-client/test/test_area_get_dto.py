# coding: utf-8

"""
    OpenSilex API

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: 1.4.7-rdg
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from openapi_client.models.area_get_dto import AreaGetDTO

class TestAreaGetDTO(unittest.TestCase):
    """AreaGetDTO unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> AreaGetDTO:
        """Test AreaGetDTO
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `AreaGetDTO`
        """
        model = AreaGetDTO()
        if include_optional:
            return AreaGetDTO(
                uri = '',
                rdf_type = '',
                is_structural_area = True,
                name = '',
                description = '',
                publisher = openapi_client.models.user_get_dto.UserGetDTO(
                    uri = 'http://opensilex.dev/users#jean.michel.inrae', 
                    email = 'jean.michel@example.com', 
                    language = 'en', 
                    admin = False, 
                    first_name = 'Jean', 
                    last_name = 'Michel', 
                    linked_person = 'http://opensilex.dev/person#Jean.Michel.mistea', 
                    enable = True, 
                    favorites = test, ),
                geometry = {"coordinates":[43.618316,3.856912],"type":"Point"},
                event = openapi_client.models.event_get_dto.EventGetDTO(
                    uri = 'http://www.opensilex.org/event/12590c87-1c34-426b-a231-beb7acb33415', 
                    publisher = openapi_client.models.user_get_dto.UserGetDTO(
                        uri = 'http://opensilex.dev/users#jean.michel.inrae', 
                        email = 'jean.michel@example.com', 
                        language = 'en', 
                        admin = False, 
                        first_name = 'Jean', 
                        last_name = 'Michel', 
                        linked_person = 'http://opensilex.dev/person#Jean.Michel.mistea', 
                        enable = True, 
                        favorites = test, ), 
                    rdf_type = 'http://www.opensilex.org/vocabulary/oeev#Irrigation', 
                    rdf_type_name = 'Move', 
                    start = '2019-09-08T12:00:00+01:00', 
                    end = '2019-09-08T12:00:00+01:00', 
                    is_instant = False, 
                    description = 'The pest attack lasted 20 minutes', 
                    targets = [
                        ''
                        ], 
                    publication_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    last_updated_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), ),
                publication_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                last_updated_date = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f')
            )
        else:
            return AreaGetDTO(
        )
        """

    def testAreaGetDTO(self):
        """Test AreaGetDTO"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
